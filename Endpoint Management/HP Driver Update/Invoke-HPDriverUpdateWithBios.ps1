...
.NOTES
	...
	1.0.3 Modified - (2022-05-17) Since for some reason HPIA doesn't update the bios, it's now done via HPCMSL Get-HPBIOSUpdates cmdlet, 
								  also added support for exit code 257.
								  + other changes like a fix for packagemanager ps module v1.0.0.1 in programfiles prevents updating the module is handled,
								  the script will always overwrite itself in windows temp dir, and delete itself only if running form temp dir.
#>
            ...
            "Execute" {
                try {
					...
					try {
						...
						# Delete Version 1.0.0.1 module folders, this prevents powershell from using updated cmdlets  
						if(Test-Path 'C:\Program Files (x86)\WindowsPowerShell\Modules\PackageManagement\1.0.0.1'){Remove-Item 'C:\Program Files (x86)\WindowsPowerShell\Modules\PackageManagement\1.0.0.1' -Force -Recurse}
						if(Test-Path 'C:\Program Files\WindowsPowerShell\Modules\PackageManagement\1.0.0.1'){Remove-Item 'C:\Program Files\WindowsPowerShell\Modules\PackageManagement\1.0.0.1' -Force -Recurse}
						if(Test-Path 'C:\Program Files (x86)\WindowsPowerShell\Modules\PowerShellGet\1.0.0.1'){Remove-Item 'C:\Program Files (x86)\WindowsPowerShell\Modules\PowerShellGet\1.0.0.1' -Force -Recurse}
						if(Test-Path 'C:\Program Files\WindowsPowerShell\Modules\PowerShellGet\1.0.0.1'){Remove-Item 'C:\Program Files\WindowsPowerShell\Modules\PowerShellGet\1.0.0.1' -Force -Recurse}
                    ...
                    try {
                        ...
                        try {
                            ...
                            switch ($Invocation) {
								...
								257 { # The analysis returned no recommendations
                                    Write-LogEntry -Value "HP Image Assistant probably found a BIOS update, exit code: $($Invocation)" -Severity 1
                                    Set-RegistryValue -Path "HKLM:\SOFTWARE\HP\ImageAssistant" -Name "ExecutionResult" -Value "Success" -ErrorAction Stop
                                }
                            ...
                            # Use HPCMSL to update the BIOS
							$InstalledBIOS = [Version](Get-HPBiosVersion)
							$LatestBIOS = [Version]((Get-HPBIOSUpdates -Latest).Ver)
							if($InstalledBIOS -lt $LatestBIOS)
							{
								Write-LogEntry -Value "Using alternative method to update the bios (CMSL Get-HPBIOSUpdates)" -Severity 1
								Write-LogEntry -Value "BIOS version will be updated from $InstalledBIOS to $LatestBIOS" -Severity 1
								try
								{
									Get-HPBIOSUpdates -Flash -Bitlocker Suspend -Force -Yes
									Write-LogEntry -Value "New BIOS will be applied during next system boot." -Severity 1
									if($ExitCode -eq 0){$ExitCode = 3010}
								}
								catch
								{
									Write-LogEntry -Value "Something went wrong, failed to update the BIOS." -Severity 3
									Set-RegistryValue -Path "HKLM:\SOFTWARE\HP\ImageAssistant" -Name "ExecutionResult" -Value "Failed" -ErrorAction Stop
									$ExitCode = 1
								}
							}
                            ...
                            $TempDirScriptPath = "$WindowsTempPath\$($MyInvocation.MyCommand.Name)"
                            Write-LogEntry -Value "Attempting to remove script from temp directory: $TempDirScriptPath" -Severity 1
                            Remove-Item -Path "$TempDirScriptPath" -Force -Confirm:$false
